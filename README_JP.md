...
既定の外部 executor は、セキュリティとクリーンな実行を確保するために、静音ラッパー [`scripts/executor_quiet.sh`](./scripts/executor_quiet.sh) 経由で実行します。このラッパーは以下の挙動を示します：

* **成功時ログの削除**: executor コマンドが正常終了（終了コード 0）した場合、生成された標準出力（stdout）および標準エラー出力（stderr）のログファイルは自動的かつ即座に削除され、ファイルシステムを汚しません。
* **失敗時ログの 600 権限とパス保持**: executor コマンドが失敗（非ゼロの終了コード）した場合、ラッパーは stdout と stderr の両方のログを保持し、`umask 077` による事前作成および明示的な `chmod 600` により、ファイルのアクセス権限を所有者のみが読み書き可能な `600` に制限した上で、ログファイルへのパスと終了コードを標準エラー出力に表示します。
* **TMPDIR フォールバック**: 環境変数 `TMPDIR` を検証してログ出力先ディレクトリを決定します。もし `TMPDIR` が未設定、空、あるいは存在しないか書き込み不可なディレクトリを指している場合は、自動的に `/tmp` にフォールバックします。`/tmp` も利用不可あるいは書き込み不可な場合は、`mktemp -d` によって作成されるプライベートな一時ディレクトリにフォールバックします。
* **作業ディレクトリに依存しないコマンド解決**: 異なるディレクトリから実行された際のコマンドパス破損を防ぐため、`executors.yaml` は `{skill_dir}` プレースホルダー（例: `{skill_dir}/scripts/executor_quiet.sh`）を使用し、Lead は実行前にこれを `executors.yaml` が存在する絶対ディレクトリに展開します。
...
停止時の要約は、最初に「直ちに止まっている理由」と「再開に必要な正確な操作」を示します。次に契約パスと task 進捗を示し、モデル名、route 比較、quota 根拠、検証コマンド全文は最後の詳細へ集約します。承認・振り分けの判断をすぐ行え、監査情報も失いません。
